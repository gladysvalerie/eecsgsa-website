      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {cardData.map((card, index) => {
            const baseClassName = `card flex flex-col cursor-pointer justify-between relative aspect-[4/3] min-h-[200px] w-full max-w-full p-5 rounded-[20px] border border-solid font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.15)] ${
            ...
            const cardStyle = {
              backgroundImage: `
                linear-gradient(
                  rgba(6, 0, 16, 0.6),   /* top overlay tint */
                  rgba(6, 0, 16, 1)    /* bottom overlay tint */
                ),
                url(${card.image || '/path/to/default.jpg'})
              `,
            ...
            if (enableStars) {
              return (
                <ParticleCard
                    onClick={() => navigate(`/about/${card.ref.toLowerCase()}`)}
                    key={index}
