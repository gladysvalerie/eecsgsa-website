...
export default function ResourcesLink() {
    const resourceLinks = {
        ...
        workPermit: {
          title: "Work Permit",
          href: "https://eecsgsa.org/office-hours",
        },
    }
...
function InfoCard({
  ...
  return (
    <Wrapper
        ...
        `}
    >
      <h2 className="text-3xl md:text-4xl lg:text-6xl font-bold mb-4">{title}</h2>
        <ul className="space-y-2 text-black">
            {items.map((item, index) => (
              <li key={index} className="flex items-start text-2xl md:text-3xl">
                {item}
              </li>
            ))}
        </ul>
